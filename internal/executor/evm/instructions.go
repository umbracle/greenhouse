...
func opExp(c *state) {
	...
	var gas uint64
	if c.isEnabled(evmc.TangerineWhistle) {
		gas = 50
	} else {
		gas = 10
	}
...
func opShl(c *state) {
	if !c.isEnabled(evmc.Constantinople) {
...
func opSload(c *state) {
	...
	var gas uint64
	if c.isEnabled(evmc.Istanbul) {
		// eip-1884
		gas = 800
	} else if c.isEnabled(evmc.TangerineWhistle) {
		gas = 200
	} else {
		gas = 50
	}
	...
	val := c.host.GetStorage(c.recipient, bigToHash(loc))
	loc.SetBytes(hashToBytes(val))
}
...
func opSStore(c *state) {
	...
	if c.isEnabled(evmc.Istanbul) && c.gas <= 2300 {
		c.exit(errOutOfGas)
		return
	}
	...
	legacyGasMetering := !c.isEnabled(evmc.Istanbul) && (c.isEnabled(evmc.Petersburg) || !c.isEnabled(evmc.Petersburg))

	status := c.host.SetStorage(c.recipient, key, val)
	cost := uint64(0)
...
func opAddress(c *state) {
	c.push1().SetBytes(addrToBytes(c.recipient))
}
...
func opBalance(c *state) {
	...
	c.push1().SetBytes(hashToBytes(c.host.GetBalance(addr)))
}
...
func opSelfBalance(c *state) {
	...
	c.push1().SetBytes(hashToBytes(c.host.GetBalance(c.recipient)))
}
...
func opChainID(c *state) {
	...
	c.push1().SetBytes(hashToBytes(c.host.GetTxContext().ChainID))
}

func opOrigin(c *state) {
	c.push1().SetBytes(addrToBytes(c.host.GetTxContext().Origin))
}

func opCaller(c *state) {
	c.push1().SetBytes(addrToBytes(c.sender))
}

func opCallValue(c *state) {
	c.push1().SetBytes(hashToBytes(c.value))
}
...
func opGasPrice(c *state) {
	c.push1().SetBytes(hashToBytes(c.host.GetTxContext().GasPrice))
}

func opReturnDataSize(c *state) {
	if !c.isEnabled(evmc.Byzantium) {
		c.exit(errOpCodeNotFound)
	} else {
...
func opExtCodeHash(c *state) {
	...
	if c.host.AccountExists(address) {
		...
		v.SetBytes(hashToBytes(c.host.GetCodeHash(address)))
	}
}
...
func opBlockHash(c *state) {
	...
	if lastBlock-257 < n && n < lastBlock {
		num.SetBytes(hashToBytes(c.host.GetBlockHash(n)))
	} else {
		num.Set(zero)
	}
}
...
func opCoinbase(c *state) {
	c.push1().SetBytes(addrToBytes(c.host.GetTxContext().Coinbase))
}
...
func opDifficulty(c *state) {
	c.push1().SetBytes(hashToBytes(c.host.GetTxContext().Difficulty))
}
...
func opSelfDestruct(c *state) {
	...
	c.host.Selfdestruct(c.recipient, address)
	c.halt()
}
...
func opLog(size int) instruction {
	...
	return func(c *state) {
		...
		c.host.EmitLog(c.recipient, topics, c.tmp)
...
func opCreate(op OpCode) instruction {
	return func(c *state) {
		...
		/*
			endowment := c.pop()
			initCodeOffset := c.pop()
			initCodeSize := c.pop()

			if !c.checkMemory(initCodeOffset, initCodeSize) {
				c.exit(errOutOfGas)
				return
			}
			...
			if op == CREATE2 {
				salt = c.pop()
				if !c.consumeGas(((initCodeSize.Uint64() + 31) / 32) * sha3WordGas) {
					c.exit(errOutOfGas)
					return
				}
			}
...
func opCall(op OpCode) instruction {
	return func(c *state) {
		...
		/*
			...
			if op == DELEGATECALL && !c.isEnabled(evmc.Homestead) {
				c.exit(errOpCodeNotFound)
				return
			}
			if op == STATICCALL && !c.isEnabled(evmc.Byzantium) {
				c.exit(errOpCodeNotFound)
				return
			}
...
func (c *state) buildCallContract(op OpCode) (*runtime.Contract, uint64, uint64, error) {
	...
	eip158 := c.isEnabled(evmc.TangerineWhistle)
	transfersValue := (op == CALL || op == CALLCODE) && value != nil && value.Sign() != 0

	if op == CALL {
		if eip158 {
			if transfersValue && c.host.AccountExists(addr) {
				gasCost += 25000
			}
	...
	if transfersValue {
		if c.host.GetBalance(c.recipient).Cmp(value) < 0 {
...
func (c *state) buildCreateContract(op OpCode) (*runtime.Contract, error) {
	...
	if c.isEnabled(evmc.TangerineWhistle) || op == CREATE2 {
		gas -= gas / 64
	}
	...
	if op == CREATE {
		address = helper.CreateAddress(c.recipient, c.host.GetNonce(c.recipient))
	} else {
		address = helper.CreateAddress2(c.recipient, bigToHash(salt), input)
	}
	contract := runtime.NewContractCreation(c.msg.Depth+1, c.msg.Origin, c.recipient, address, value, gas, input)
	return contract, nil
}
*/
...
func opHalt(op OpCode) instruction {
	return func(c *state) {
		if op == REVERT && !c.isEnabled(evmc.Byzantium) {
			c.exit(errOpCodeNotFound)
			return
		}
...
func hashToBytes(hash evmc.Hash) []byte {
	return hash[:]
}

func addrToBytes(addr evmc.Address) []byte {
	return addr[:]
}
