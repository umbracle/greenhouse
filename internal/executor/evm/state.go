...
var (
	errOutOfGas              = errors.New("out of gas")         /*runtime.ErrOutOfGas*/
	errStackUnderflow        = errors.New("stack underflow")    /*runtime.ErrStackUnderflow*/
	errStackOverflow         = errors.New("stack overflow")     /*runtime.ErrStackOverflow*/
	errRevert                = errors.New("execution reverted") /*runtime.ErrExecutionReverted*/
...
func (c *state) popHash() evmc.Hash {
	return bytesToHash(c.pop().Bytes())
}
...
func (c *state) popAddr() (evmc.Address, bool) {
	...
	return bytesToAddress(b.Bytes()), true
}
...
func (c *state) isEnabled(rev evmc.Revision) bool {
	return rev >= c.rev
}
...
func bigToHash(b *big.Int) evmc.Hash {
...
func bytesToAddress(b []byte) evmc.Address {
	var a evmc.Address

	size := uint64(len(b))
	min := min(size, 20)

	copy(a[20-min:], b[size-min:])
	return a
}
